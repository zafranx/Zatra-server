            ...
            if (
                ...
                for (const user of adminUsers) {
                    try {
                        ...
                        console.log(
                            `Creating Zatra login for UserId: ${user._id}`
                        );
                        ...
                        console.error(
                            "⚠️ Failed to create ZatraAdmin login:",
                            err.message
                        );
                    }
                }
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            console.error(" SaveZatra Error:", error);
            return res.json(
                __requestResponse("500", __SOME_ERROR, error.message)
            );
        }
    }
);
...
router.post("/ListZatra-EnrouteStations", async (req, res) => {
    try {
        ...
        if (!ZatraId || !mongoose.Types.ObjectId.isValid(ZatraId)) {
            return res.json(
                __requestResponse("400", "Invalid or missing ZatraId")
            );
        }
        ...
        if (list.length === 0) {
            return res.json(
                __requestResponse("404", "No EnrouteStations found", [])
            );
        }
    ...
    async (req, res) => {
        try {
            ...
            if (!_id || !mongoose.Types.ObjectId.isValid(_id)) {
                return res.json(
                    __requestResponse("400", "Invalid ZatraId", {})
                );
            }
        ...
        const processAdmins = async (ids, role, label) => {
            ...
            results.forEach((r, i) => {
                if (r.status === "rejected") {
                    ...
                    console.log(
                        `✅ ${label} login created for userId ${users[i]._id}`
                    );
                }
            });
        };
