...
router.post("/login", async (req, res) => {
    try {
        ...
        if (!Password?.trim()) {
            return res.json(
                __requestResponse("400", "Please provide a password.")
            );
        }
        ...
        return res.json(
            __requestResponse("200", "Login successful.", response)
        );
    } catch (error) {
        console.error("Error in login:", error);
        return res.json(
            __requestResponse("500", "Internal server error.", error)
        );
    }
});
...
router.post("/login3", async (req, res) => {
    try {
        ...
        // Dynamically construct response data for the specific asset type
        const assetData = CheckUser?.AssetId?._doc || {};
        const assetSpecificData = isAdmin
            ? {} // No asset-specific data for Super Admin
            : {
                  [assetType]: assetData[assetType]
                      ? {
                            ...assetData[assetType],
                            ProfilePic: assetData[assetType]?.ProfilePic
                                ? `${imageBasePath}${assetData[assetType]?.ProfilePic}`
                                : "",
                        }
                      : {},
              };
        ...
        const user = isAdmin
            ...
            : {
                  ...
                  ...assetSpecificData, // Include only relevant asset-specific data
              };
...
router.post("/login4x", async (req, res) => {
    try {
        const { LoginId, Password } = req.body;
        ...
        // Determine asset type or super admin
        const isAdmin = CheckUser.IsAdmin === true;
        const assetType = isAdmin
            ? "SuperAdmin"
            : CheckUser?.AssetId?.AssetTypeID?.lookup_value || "Unknown";

        // Dynamically construct response using deep clone
        const user = isAdmin
            ? { Role: "SuperAdmin" }
            : __deepClone(CheckUser.AssetId)
            ? {
                  _id: CheckUser.AssetId?._id,
                  AssetName: CheckUser.AssetId?.AssetName,
                  AssetTypeID: CheckUser.AssetId?.AssetTypeID,
...
router.post("/createLogin", async (req, res) => {
    try {
        ...
        if (existingLogin) {
            return res.json(
                __requestResponse("400", "Login ID already exists.")
            );
        }
    ...
    try {
        ...
        if (!LoginId || !NewPassword) {
            return res.json(
                __requestResponse(
                    "400",
                    "LoginId and New Password are required."
                )
            );
        }
        ...
        return res.json(
            __requestResponse("200", "Password reset successfully.")
        );
    } catch (error) {
